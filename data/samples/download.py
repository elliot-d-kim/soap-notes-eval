"""Download 2-3 representative samples from each source dataset.

Datasets:
- adesouza1/soap_notes  (HuggingFace)
- ACI-Bench/Benchmark-LLM-Clinical-Note-Generation  (HuggingFace)
- omi-health/medical-dialogue-to-soap-summary  (HuggingFace)
...
def download_adesouza() -> list[str]:
    ...
    print("\n[1/3] adesouza1/soap_notes")
    saved = []
    try:
        ds = load_dataset("adesouza1/soap_notes", split="train", streaming=True, trust_remote_code=True)
        for i, row in enumerate(ds):
            if i >= N_SAMPLES:
                break
            note_id = f"adesouza_{i:03d}"
            # Field names vary — preserve all, add canonical aliases
            record = dict(row)
            # Map to canonical names while keeping originals
            note_text = record.get("soap_note", record.get("note", record.get("output", "")))
            transcript = record.get("transcript", record.get("dialogue", record.get("input", "")))
        ...
        # Create a synthetic fallback sample so the pipeline can still run
...
def download_aci_bench() -> list[str]:
    """Download samples from ACI-Bench."""
    print("\n[2/3] ACI-Bench")
    saved = []
    try:
        ds = load_dataset(
            "ACI-Bench/Benchmark-LLM-Clinical-Note-Generation",
            split="test",
            streaming=True,
        ...
        for i, row in enumerate(ds):
            if i >= N_SAMPLES:
                break
            note_id = f"aci_{i:03d}"
            record = dict(row)
            # ACI-Bench fields: 'dialogue', 'note', sometimes 'reference'
            note_text = record.get("note", record.get("clinical_note", record.get("reference", "")))
            transcript = record.get("dialogue", record.get("conversation", ""))
            record["note_text"] = note_text
            record["transcript"] = transcript
            path = SAMPLES_DIR / f"{note_id}.json"
            save(record, path)
        ...
        print(f"  WARNING: could not download ACI-Bench — {e}")
        saved.extend(_create_fallback_samples("aci", N_SAMPLES))
    return saved
...
def download_omi_health() -> list[str]:
    ...
    print("\n[3/3] omi-health/medical-dialogue-to-soap-summary")
    saved = []
    try:
        ds = load_dataset(
        ...
        for i, row in enumerate(ds):
            ...
            record = dict(row)
            # Omi fields: 'conversation'/'dialogue', 'soap_summary'/'soap_note'
            note_text = record.get(
                "soap_summary",
                record.get("soap_note", record.get("note", record.get("output", ""))),
            )
            transcript = record.get(
                "conversation", record.get("dialogue", record.get("input", ""))
            )
            record["note_text"] = note_text
            record["transcript"] = transcript
            path = SAMPLES_DIR / f"{note_id}.json"
...
def write_manifest(
    ...
    for note_id in aci_ids:
        entries.append({
            "note_id": note_id,
            "filename": f"{note_id}.json",
            "source_dataset": "ACI-Bench",
            "label": "good",
            "metadata": {},
        })
...
if __name__ == "__main__":
    ...
    aci_ids = download_aci_bench()
    omi_ids = download_omi_health()
    write_manifest(adesouza_ids, aci_ids, omi_ids)
    print("\nDone.")
